...
import { HyperlinkOnClick } from "./links/hyperlink-on-click";
...
function createExternalDrawing(drawingOptions?: IDrawingOptions): Drawing {
    return new Drawing(
        {
...
describe("Drawing", () => {
    ...
    describe("#hyperlinkOnClick()", () => {
        it("should create a drawing with inline element with hyperlink on click", () => {
            currentBreak = createDrawing();
            currentBreak.addHyperlinkOnClick(new HyperlinkOnClick(0));
            const tree = new Formatter().format(currentBreak);
            expect(tree).to.deep.equal({
                "w:drawing": [
                    {
